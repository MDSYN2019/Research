...
real(8), dimension(:,:,:,:), allocatable :: Rreference
real(8), dimension(:), allocatable :: UIns
           ...
           do kk = 1 , NumOriTry
             ...
             do l = 1 , NumSpecInsert
               ...
               do jj = 1 , NumAtmInsert(l)
                 ...
                 Rmolec(:,jj,l) = Rreference(:,jj,l,iran)
...
!   print *, 'kT=',kT
   ...
   do i = 1 , NumSpecInsert
     ...
     write(String,'(a,i1,a)') './Analy/TestUInsert_',i,'.data'
     ...
     do iz = 1 , lz
       ...
       CPot   = AveEneZPT / AveEneZ

       if(lzh*2==lz) then

         write(51,'(f7.2,2e14.6)') (iz-lzh-0.5)*R_cav, CPot

       else

         write(51,'(f7.2,2e14.6)') (iz-lzh-1.0)*R_cav, CPot
...
character(len=4) :: tname
   ...
   allocate( Rreference(3,MaxA,NumSpecInsert,1000) )
...
!   allocate( R_onMol(3,MaxA,NumSpecInsert) )
   ...
   do i = 1 , NumSpecInsert
     ...
     write(filename,'(a,a)') trim(MolNameCBPI(i)),'.xyz'

     open(52,file=trim(filename))
     do j = 1, 1000
       read(52,*) 
       read(52,*) 
       do k = 1, NumAtmInsert(i)
         read(52,*) tname,Rreference(:,k,i,j)
       end do
     end do
     close(52)
