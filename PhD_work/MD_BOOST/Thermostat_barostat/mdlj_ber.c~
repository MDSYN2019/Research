/* 
   ...
   in a periodic boundary using the Berendsen Thermostat
...
int main ( int argc, char * argv[] ) {
  ...
  int N=216,c,a;
  double L=0.0;
  double rho=0.5, Tb = 1.0, tau=1.0, rc2 = 1.e20;
  double vir, vir_sum, pcor, V;
  double PE, KE, TE, ecor, ecut, T0=0.0, TE0;
  double rr3,dt=0.001, dt2, lambda;
  int i,j,s;
  ...
  fprintf(stdout,"# Berendsen-Thermostat MD Simulation"
	  " of a Lennard-Jones fluid\n");
  fprintf(stdout,"# L = %.5lf; rho = %.5lf; N = %i; rc = %.5lf\n",
	  L,rho,N,sqrt(rc2));
  fprintf(stdout,"# nSteps %i, seed %d, dt %.5lf, T0 %.5lf, tau %.5lf\n",
	  nSteps,Seed,dt,T0,tau);
  ...
  for (s=0;s<nSteps;s++) {
    ...
    lambda = sqrt(1+dt/tau*(Tb/(2.0*KE/3.0/N)-1.0));
    KE = 0.0;
    for (i=0;i<N;i++) {
      vx[i]*=lambda;
      vy[i]*=lambda;
