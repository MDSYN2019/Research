...
const int n_atoms = 25; // Set the number of atoms in the box
const int num_moves = 20000000; // Set the number of Monte Carlo moves to perform
double box_size[3] = { 15.0, 15.0, 15.0 }; // Set the size of the box (in Angstroms)
const double max_translate = 0.5;   // angstroms; // The maximum amount that the atom can be translated by
const double temperature = 100;   // kelvin // Simulation temperature
double pressure = 1 * 1.458397506863647E-005;   // atmospheres // Simulation pressure (atmospheres converted to internal // units - kcal mol-1 A-3)
			...
			const double sigma, const double epsilon)
{
  ...
  for (int i = 0; i < n_atoms-1; i = i + 1) // ith pair 
    {
      for (int j = i+1; j < n_atoms; j = j + 1) // jth pair 
	{
	  double delta_x = coords[j][0] - coords[i][0]; // Distance of x coordinates
	  double delta_y = coords[j][1] - coords[i][1]; // Distance of y coordinates
	  double delta_z = coords[j][2] - coords[i][2]; // Distance of z coordinates
