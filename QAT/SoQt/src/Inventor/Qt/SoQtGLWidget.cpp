...
SoQtGLWidgetP::eventFilter(QObject * obj, QEvent * e)
{
  if (SOQT_DEBUG && 0) { // debug
#if QT_VERSION >= 0x040000
    SbString w = obj->objectName().toStdString().c_str();
#else
    SbString w = obj->name();
#endif
  ...
  int kbdevent = FALSE;
  if (/*QEvent::KeyPress*/ 6 == etype ) kbdevent = TRUE;
  if (/*QEvent::KeyRelease*/7 == etype ) kbdevent = TRUE;
...
SoQtGLWidgetP::buildGLWidget(void)
{
  ...
  if (wascurrent) {
    ...
    // JFB this seems to be unnecessary
